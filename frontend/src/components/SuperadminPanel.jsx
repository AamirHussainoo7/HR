...
import {
  ShieldCheck, UserPlus, RefreshCw, UserMinus, CheckCircle, XSquare, AlertCircle, HelpCircle
} from "lucide-react";
...
export default function SuperadminPanel({ authToken }) {
  ...
  const handleInvite = async (e) => {
    ...
    } catch (err) { console.error(err); setAlertMsg("Invite failed."); }
      ...
      <div className="glass p-6 rounded-xl space-y-4 bg-zinc-900/10">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={inviteRole} onChange={(e) => setInviteRole(e.target.value)}
                className="w-full px-2.5 py-1.5 bg-zinc-900 border border-zinc-800 text-zinc-100 rounded-lg text-xs font-semibold focus:outline-none">
            ...
            <div>
              ...
              <input type="text" required value={inviteDept} onChange={(e) => setInviteDept(e.target.value)}
                className="w-full px-3 py-1.5 bg-zinc-900/50 border border-zinc-800 text-zinc-100 rounded-lg text-xs" />
            </div>
          </div>
          <button type="submit" disabled={isInviting}
            className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs rounded-lg transition-colors flex items-center justify-center gap-1 cursor-pointer">
        ...
        {alertMsg && (
          <div className={`p-3 rounded-lg text-xs font-mono font-medium border ${
            alertMsg.startsWith("Error") ? "bg-rose-950/20 text-rose-400 border-rose-900/50" : "bg-emerald-950/20 text-emerald-400 border-emerald-900/50"
          }`}>{alertMsg}</div>
        )}
      </div>

      <div className="lg:col-span-2 glass rounded-xl overflow-hidden bg-zinc-900/10">
        <div className="p-4 bg-zinc-900/20 border-b border-zinc-800 flex justify-between items-center">
          ...
          <button onClick={fetchUsers}
            ...
            <RefreshCw className="w-3 h-3 text-indigo-400" /><span>Reload Db</span>
          </button>
        </div>
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left text-zinc-400">
              <thead className="bg-[#0e0e0f] text-[10px] uppercase font-bold text-zinc-500 border-b border-zinc-800 font-mono">
                <tr>
              ...
              <tbody className="divide-y divide-zinc-900 bg-zinc-900/5">
                {users.map((usr) => (
                  <tr key={usr.id} className="hover:bg-zinc-900/20 transition-all">
                    ...
                    <td className="px-4 py-3">
                      <select value={usr.role} onChange={(e) => handleUpdateUser(usr.id, { role: e.target.value })}
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                        usr.isActive ? "bg-emerald-950/40 text-emerald-400 border border-emerald-900/50" : "bg-rose-950/40 text-rose-400 border border-rose-900/50"
                      }`}>{usr.isActive ? "ACTIVE" : "INACTIVE"}</span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      {usr.isActive ? (
                        <button type="button" onClick={() => handleUpdateUser(usr.id, { isActive: false })}
                        ...
                        <button type="button" onClick={() => handleUpdateUser(usr.id, { isActive: true })}
