...
import {
  UploadCloud, FileCheck, AlertTriangle, Sparkles, Search, Trash2,
  FileText, Check, X, Loader2, ChevronRight, Filter
} from "lucide-react";
import { apiUrl } from "../api.js";

export default function ResumeScreening({ resumes, authToken, onRefresh }) {
  ...
  const handleCreateResume = async (e) => {
    ...
    try {
      ...
      if (response.ok) { setCandidateName(""); setCandidateEmail(""); setResumeTextContent(""); onRefresh(); }
    } catch (err) { console.error(err); }
    finally { setUploading(false); }
  };

  const handleTriggerScreen = async (id) => {
    setScreeningId(id);
    try {
      ...
      if (response.ok) {
        ...
        onRefresh();
        if (selectedResume && selectedResume.id === id) setSelectedResume(data.resume);
      }
    } catch (err) { console.error(err); }
  ...
  const handleDeleteResume = async (id, e) => {
    ...
    try {
      ...
      if (response.ok) { if (selectedResume?.id === id) setSelectedResume(null); onRefresh(); }
    } catch (err) { console.error(err); }
  };

  const handleBulkAction = async () => {
    const pendingResumes = filteredResumes.filter(r => r.status === "pending");
    if (pendingResumes.length === 0) { alert("No pending profiles found for bulk updates."); return; }
    setUploading(true);
        ...
        <div className="glass p-6 rounded-xl">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-1.5">
              {["frontend", "ai"].map(prof => (
                <button key={prof} type="button" onClick={() => loadDemoResumeText(prof)}
                  ...
                  + {prof === "frontend" ? "React Dev Resume" : "AI Dev Resume"}
                </button>
              ))}
            </div>
          </div>
          ...
          <form onSubmit={handleCreateResume} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input type="text" required value={candidateName} onChange={(e) => setCandidateName(e.target.value)}
                  placeholder="e.g. Liam Neeson" className="w-full px-3 py-1.5 bg-zinc-900/50 border border-zinc-800 text-zinc-100 rounded-lg text-xs placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
              </div>
              <div>
                ...
                <input type="email" required value={candidateEmail} onChange={(e) => setCandidateEmail(e.target.value)}
                  placeholder="name@email.com" className="w-full px-3 py-1.5 bg-zinc-900/50 border border-zinc-800 text-zinc-100 rounded-lg text-xs placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea required rows={3} value={resumeTextContent} onChange={(e) => setResumeTextContent(e.target.value)}
        ...
        <div className="glass rounded-xl overflow-hidden">
          <div className="p-4 bg-zinc-900/20 border-b border-zinc-800/80 flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div className="relative flex-1 max-w-md">
              ...
              <input type="text" placeholder="Search candidates, positions, email..." value={searchTerm}
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={handleBulkAction} disabled={uploading}
          ...
          <div className="divide-y divide-zinc-800/40 max-h-[490px] overflow-y-auto">
            {filteredResumes.length === 0 ? (
              <div className="p-12 text-center text-zinc-500 text-xs">No matching profiles found. Inject new profiles above!</div>
              ...
              return (
                <div key={resume.id} onClick={() => setSelectedResume(resume)}
                  className={`p-4 flex items-center justify-between gap-4 cursor-pointer hover:bg-zinc-900/20 transition-all ${isSelected ? "bg-indigo-600/10 border-l-4 border-indigo-500" : ""}`}>
                  <div className="flex items-start gap-3">
                    <div className="p-2.5 rounded-lg bg-indigo-600/10 text-indigo-400 mt-1"><FileText className="w-5 h-5" /></div>
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-0.5 text-xs text-zinc-400 font-medium">
                        <span>{resume.email}</span><span className="text-zinc-600">•</span><span className="text-zinc-300">{resume.jobTitle}</span>
                      </div>
                      <span className="text-[10px] text-zinc-500 font-mono mt-1 block">Uploaded on {new Date(resume.uploadedAt).toLocaleDateString()}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
                    {resume.status === "pending" ? (
                      <button onClick={() => handleTriggerScreen(resume.id)} disabled={isScreening}
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white font-medium text-xs rounded shadow-sm cursor-pointer">
                        {isScreening ? <Loader2 className="w-3 animate-spin text-white" /> : <Sparkles className="w-3 text-white" />}
                        <span>{isScreening ? "Analyzing..." : "Review Profile"}</span>
                      </button>
                    ) : (
                      <div className="flex items-center gap-2">
                        {resume.aiScore !== undefined && (
                          <span className={`px-2.5 py-1 text-xs font-mono font-bold rounded-lg border ${scoreColor}`}>{resume.aiScore}/100</span>
                        )}
                        {resume.recommendation && (
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide border ${
                            resume.recommendation === "shortlist" ? "bg-emerald-950/20 text-emerald-400 border-emerald-900/30"
                            : resume.recommendation === "maybe" ? "bg-amber-950/20 text-amber-400 border-amber-900/30"
                            : "bg-rose-950/20 text-rose-400 border-rose-900/30"
                          }`}>{resume.recommendation}</span>
                        )}
                      </div>
                    )}
                    <button onClick={(e) => handleDeleteResume(resume.id, e)}
          ...
          <div className="space-y-6 h-full flex flex-col justify-between">
            <div>
              ...
              {selectedResume.status === "pending" ? (
                <div className="py-8 text-center bg-zinc-900/10 rounded-lg p-4 border border-dashed border-zinc-800 my-4">
                  ...
                  <button onClick={() => handleTriggerScreen(selectedResume.id)}
                    ...
                    <Sparkles className="w-3 h-3 text-white" /><span>Run Evaluation Now</span>
                  </button>
                </div>
                ...
                <div className="space-y-4 py-4" id="ai-insights-block">
                  <div className="p-3 bg-zinc-900/30 rounded-lg border border-zinc-800/80">
                    <span className="text-[10px] font-bold text-indigo-400 tracking-wider block uppercase font-mono mb-1">Candidate Alignment Summary</span>
                    <p className="text-xs text-zinc-300 leading-relaxed italic">&ldquo;{selectedResume.aiSummary}&rdquo;</p>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold text-zinc-500 tracking-wider block uppercase font-mono mb-2">Key Capabilities Detected</span>
                    <div className="flex flex-wrap gap-1">
                      {selectedResume.skills?.map((skill, idx) => (
                        <span key={idx} className="px-2 py-0.5 bg-indigo-600/10 border border-indigo-500/20 text-indigo-400 rounded text-[10px] font-medium">{skill}</span>
                      ))}
                    </div>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold text-zinc-500 tracking-wider block uppercase font-mono mb-1.5">Operational Concerns / Red Flags</span>
                      ...
                      <div className="space-y-1">
                        {selectedResume.redFlags.map((flag, idx) => (
                          <div key={idx} className="flex items-start gap-1.5 text-xs text-rose-400 bg-rose-950/15 px-2 py-1 rounded border border-rose-900/20">
                            <AlertTriangle className="w-3" /><span>{flag}</span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div className="flex items-start gap-2 p-2 bg-emerald-950/15 text-emerald-400 text-xs rounded border border-emerald-900/20">
                        <Check className="w-3.5 h-3.5 mt-0.5" /><span>No red flags detected. Clean profile.</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="mt-4">
                <span className="text-[10px] font-bold text-zinc-500 tracking-wider block uppercase font-mono mb-1.5">Extracted CV Context</span>
            ...
            {selectedResume.status === "processed" && (
              <div className="pt-4 border-t border-zinc-800 flex gap-2">
                <button type="button" onClick={() => alert("Successfully scheduled interview call with: " + selectedResume.candidateName)}
                  ...
                  <FileCheck className="w-3.5 h-3.5" /><span>Shortlist Profile</span>
                </button>
                <button type="button" onClick={() => alert("Candidate rejected from screening pipelines.")}
