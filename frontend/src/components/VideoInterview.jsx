...
import {
  Video, Calendar, Plus, User, CheckCircle2, Sparkles, Clock, Play,
  Mic, RefreshCw, VideoOff, Radio, FileCheck2, ChevronRight, AlertCircle
} from "lucide-react";
import { apiUrl } from "../api.js";

export default function VideoInterview({ interviews, userRole, authToken, onRefresh }) {
  ...
  useEffect(() => {
    if (isRecording && videoElementRef.current) {
      navigator.mediaDevices.getUserMedia({ video: true, audio: true })
        ...
        .catch((err) => console.warn("Camera streaming turned off or blocked: ", err));
    } else {
      stopWebcam();
    }
    return () => stopWebcam();
  ...
  useEffect(() => {
    if (isRecording && countdown > 0) {
      countdownTimerRef.current = setInterval(() => setCountdown((c) => c - 1), 1000);
  ...
  const stopWebcam = () => {
    if (userStream) { userStream.getTracks().forEach((track) => track.stop()); setUserStream(null); }
  ...
  const handleSchedule = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setCandidateEmail(""); setCandidateName(""); setScheduledAt("");
        setMsg(data.message || "Interview scheduled successfully!");
        onRefresh();
      } else { setMsg(`Error: ${data.error}`); }
    } catch (err) { console.error(err); setMsg("Failed scheduling API call."); }
    finally { setScheduling(false); }
  };

  const startQuestionAttempt = () => { setCountdown(30); setIsRecording(true); setTranscriptInput(""); };

  const handleSubmitAnswer = async () => {
    if (!selectedInterview) return;
    setProcessingAnswer(true); stopWebcam(); setIsRecording(false);
    ...
    if (!answerText) {
      ...
      answerText = sampleAnswers[selectedInterview.questions[activeQuestionIdx]?.questionId] || "We handled the project lifecycle iteratively, focusing on speed, low latency, and modular codebase components.";
    }
    try {
      ...
      const response = await fetch(apiUrl(`/api/interviews/${selectedInterview.id}/response`), {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${authToken}` },
        body: JSON.stringify({ questionId: qId, transcriptText: answerText, inlineAudioBase64: "recorded" })
      });
      if (response.ok) {
        onRefresh();
        if (activeQuestionIdx + 1 < selectedInterview.questions.length) {
          setActiveQuestionIdx(activeQuestionIdx + 1); setTranscriptInput("");
        } else {
    ...
    finally { setProcessingAnswer(false); }
  };

  const loadDynamicQuestions = async (intId) => {
    try {
      const resp = await fetch(apiUrl(`/api/interviews/${intId}/questions`), {
        headers: { "Authorization": `Bearer ${authToken}` }
      });
      if (resp.ok) { onRefresh(); alert("Success! Generated 4 contextual questions utilizing Gemini Flash!"); }
    ...
    <div className="space-y-6">
      {(userRole === "superadmin" || userRole === "hr_manager") && (
        <div className="glass p-6 rounded-xl">
          ...
          <form onSubmit={handleSchedule} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div>
              ...
              <input type="text" required value={candidateName} onChange={(e) => setCandidateName(e.target.value)}
                placeholder="David Smith" className="w-full px-3 py-1.5 bg-zinc-900/50 border border-zinc-800 text-zinc-100 rounded-lg text-xs placeholder-zinc-650 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-zinc-400 uppercase mb-1 font-mono">Candidate Email</label>
              <input type="email" required value={candidateEmail} onChange={(e) => setCandidateEmail(e.target.value)}
                placeholder="david@candidate.com" className="w-full px-3 py-1.5 bg-zinc-900/50 border border-zinc-800 text-zinc-100 rounded-lg text-xs placeholder-zinc-650 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
            </div>
            <div>
          ...
          {msg && (
            <div className={`mt-3 p-3 rounded-lg text-xs font-medium border ${msg.startsWith("Error") ? "bg-rose-950/20 text-rose-400 border-rose-900/30" : "bg-emerald-950/20 text-emerald-400 border-emerald-900/30"}`}>
              {msg}
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="glass rounded-xl overflow-hidden">
          <div className="p-4 bg-zinc-900/20 border-b border-zinc-800/80 flex justify-between items-center">
            <span className="font-sans font-bold text-zinc-100 text-sm">Active Session Listings</span>
            <span className="px-2 py-0.5 bg-zinc-800 border border-zinc-700 text-zinc-300 font-bold text-[10px] rounded-full">{interviews.length} TOTAL</span>
          </div>
          <div className="divide-y divide-zinc-800/40">
            {interviews.length === 0 ? (
              <div className="p-12 text-center text-zinc-500 text-xs">No scheduled interview campaigns found in queue.</div>
            ) : interviews.map((int) => {
              const isSelected = selectedInterview?.id === int.id;
              return (
                <div key={int.id} onClick={() => { setSelectedInterview(int); setActiveQuestionIdx(0); }}
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <h4 className="font-bold text-zinc-200 text-xs flex items-center gap-1">
                        <User className="w-3.5 h-3.5 text-zinc-500" /><span>{int.candidateName}</span>
                      </h4>
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <span className={`px-2 py-0.5 text-[9px] font-bold uppercase rounded border ${
                        int.status === "completed" ? "bg-emerald-950/20 text-emerald-400 border-emerald-900/30"
                        : int.status === "in_progress" ? "bg-amber-950/20 text-amber-400 border-amber-900/30"
                        : "bg-indigo-950/25 text-indigo-400 border-indigo-900/20"
                      }`}>{int.status}</span>
                      {int.overallScore && (
                        <span className="text-[10px] bg-zinc-800 border border-zinc-700 text-zinc-300 px-1 py-0.5 font-bold font-mono rounded block mt-1">{int.overallScore}% score</span>
                      )}
                    </div>
                  </div>
                  {(userRole === "superadmin" || userRole === "hr_manager") && (
                    <div className="mt-3 pt-2.5 border-t border-zinc-800/60 flex justify-end">
                      <button type="button" onClick={(e) => { e.stopPropagation(); loadDynamicQuestions(int.id); }}
                        ...
                        <Sparkles className="w-2.5 h-2.5" /><span>Regenerate Custom Questions</span>
                      </button>
                    </div>
                  )}
                </div>
              ...
              {userRole === "candidate" && selectedInterview.status !== "completed" ? (
                <div className="space-y-6">
                  <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl text-white flex justify-between items-center">
                    <div>
                      <span className="text-[9px] font-bold tracking-widest text-indigo-400 uppercase block font-mono">Candidate Interview Chamber</span>
                  ...
                  <div className="bg-zinc-900/40 border border-zinc-800/80 p-4 rounded-lg">
                    ...
                    <p className="font-sans font-extrabold text-zinc-200 text-base leading-snug">
                      &ldquo;{selectedInterview.questions[activeQuestionIdx]?.questionText}&rdquo;
                    </p>
                  </div>
                  ...
                  <div className="relative bg-zinc-950 rounded-xl overflow-hidden aspect-video border border-zinc-900 flex flex-col justify-between items-center" id="camcorder-frame">
                    {isRecording ? (
                      <video ref={videoElementRef} muted className="w-full h-full object-cover rounded-xl" />
                    ) : (
                      <div className="flex flex-col items-center justify-center text-zinc-500 text-center p-8 m-auto">
                        <VideoOff className="w-12 h-12 mb-2 text-zinc-600" />
                        <span className="text-xs">Webcam feed inactive. Tap &quot;Begin Response&quot; to enable camera.</span>
                      </div>
                    )}
                    {isRecording && (
                      <div className="absolute top-4 right-4 bg-rose-600/90 text-white font-bold font-mono text-xs px-3 py-1 rounded-full flex items-center gap-1.5">
                        <Radio className="w-3.5 h-3.5 animate-pulse" /><span>RECORDING • {countdown}s</span>
                      </div>
                    )}
                  </div>
                  ...
                  {isRecording && (
                    <div className="space-y-2">
                      <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-wider font-mono">🎤 Simulated Answer Submission (Voice / Text Sandbox)</label>
                      <textarea rows={2} value={transcriptInput} onChange={(e) => setTranscriptInput(e.target.value)}
                        placeholder="Say or type your answer here..."
                        className="w-full px-3 py-2 bg-zinc-900/50 border border-zinc-800 text-zinc-100 rounded-lg text-xs font-mono" />
                    </div>
                  )}

                  <div className="flex justify-end gap-3 pt-2">
                    {!isRecording ? (
                      <button type="button" onClick={startQuestionAttempt}
                        ...
                        <Play className="w-4 h-4 text-white" /><span>Begin Response</span>
                      </button>
                    ) : (
                      <button type="button" onClick={handleSubmitAnswer} disabled={processingAnswer}
                        ...
                        {processingAnswer ? <RefreshCw className="w-4 h-4 animate-spin text-white" /> : <CheckCircle2 className="w-4 h-4 text-white" />}
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-start pb-4 border-b border-zinc-800">
                    <div>
                      <span className="text-[10px] font-bold text-indigo-400 tracking-wider font-mono uppercase">Scorecard &amp; Response Transcripts</span>
                    ...
                    {selectedInterview.overallScore && (
                      <div className="text-right">
                        ...
                        <span className="text-2xl font-black font-sans text-indigo-400 font-mono leading-none">{selectedInterview.overallScore}%</span>
                      </div>
                    )}
                  </div>
                  ...
                  {selectedInterview.responses.length === 0 ? (
                    ...
                    <div className="space-y-5">
                      {selectedInterview.responses.map((resp, idx) => {
                        ...
                        return (
                          <div key={idx} className="border border-zinc-800/80 rounded-xl bg-zinc-900/10 p-4 space-y-3">
                            <div className="flex justify-between items-start">
                              <span className="px-2 py-0.5 bg-zinc-800 text-zinc-300 text-[10px] font-bold rounded">ANSWER {idx + 1}</span>
                              <span className="px-2 py-0.5 bg-indigo-600/10 text-indigo-400 font-bold text-[10px] border border-indigo-500/20 rounded font-mono">SCORE: {resp.aiScore}/100</span>
                            </div>
                            <p className="text-xs font-semibold text-zinc-300 italic">Q: &ldquo;{question ? question.questionText : "Behavioral check"}&rdquo;</p>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
                              <div className="bg-zinc-950/40 p-3 rounded-lg border border-zinc-800/65">
                                <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block font-mono mb-1">Recorded Speech Transcript</span>
                                <p className="text-xs text-zinc-300 leading-relaxed font-mono">&ldquo;{resp.transcript}&rdquo;</p>
                              </div>
                              <div className="bg-indigo-600/5 p-3 rounded-lg border border-indigo-500/10">
                                <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider flex items-center gap-1 font-mono mb-1">
                                  <Sparkles className="w-3 h-3" /><span>Structured Performance Assessment</span>
                                </span>
                                <p className="text-xs text-zinc-300 leading-relaxed italic">{resp.aiFeedback}</p>
                              </div>
                            </div>
                          </div>
                        );
                  ...
                  {selectedInterview.status === "completed" && (
                    <div className="p-4 bg-emerald-950/20 rounded-xl text-emerald-400 text-xs border border-emerald-900/30 flex items-center gap-2">
                      ...
                      <span>This interview loop has been successfully evaluated. Solid suitability demonstrated!</span>
                    </div>
                  )}
                </div>
              )}
